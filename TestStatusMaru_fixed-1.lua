...
--   Fix: init order, race condition, task.wait type
-- ══════════════════════════════════════════

task.wait(1) -- FIX: was task.wait("1") string type

-- Load GUI library FIRST (blocking)
...
-- FIX: Ensure game + player + character are fully ready BEFORE init
local Players = game:GetService("Players")
local plr_init = Players.LocalPlayer

repeat task.wait(0.1) until game:IsLoaded()
repeat task.wait(0.1) until plr_init and plr_init.Character
repeat task.wait(0.1) until plr_init.Character:FindFirstChild("HumanoidRootPart")
repeat task.wait(0.1) until plr_init:FindFirstChild("Data") and plr_init.Data:FindFirstChild("Level")

-- FIX: Wait for loading screen to actually disappear
repeat task.wait(0.1) until
    plr_init.PlayerGui:FindFirstChild("Main") and
    plr_init.PlayerGui.Main:FindFirstChild("Loading")

repeat task.wait(0.1) until
    not plr_init.PlayerGui.Main.Loading.Visible or game:IsLoaded()

-- ══ GLOBAL INIT (safe now) ══
do
  ply = Players
  plr = ply.LocalPlayer
...
-- FIX: Proper loading check - wait for Loading UI to hide
repeat
    task.wait(0.1)
until (not plr.PlayerGui:FindFirstChild("Main") or
       not plr.PlayerGui.Main:FindFirstChild("Loading") or
       not plr.PlayerGui.Main.Loading.Visible) and game:IsLoaded()

-- Secondary loader in spawn (non-blocking)
task.spawn(function()
    ...
    if not success then
        warn("[MaruHub] Auto-load failed:", err)
    end
end)
